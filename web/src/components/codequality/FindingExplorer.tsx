...
export function FindingExplorer({ findings }: { findings: Finding[] }) {
  ...
  useEffect(() => setSelected((current) => current ? visible.find((finding) => finding.id === current.id && finding.dedupKey === current.dedupKey) ?? null : null), [visible])

  return <Card title="Security findings" actions={<Pill>{findings.length.toLocaleString()} findings</Pill>} bodyClass="p-0">
