...
var explicitInventory = []string{
	...
	"dockerfile-add-instead-of-copy", "dockerfile-add-remote-url", "dockerfile-apk-no-cache", "dockerfile-apt-no-assume-yes", "dockerfile-apt-no-clean", "dockerfile-apt-no-recommends", "dockerfile-apt-no-version-pin", "dockerfile-apt-upgrade", "dockerfile-copy-recursive-root", "dockerfile-entrypoint-shell-form", "dockerfile-env-legacy-format", "dockerfile-expose-invalid-port", "dockerfile-expose-ssh", "dockerfile-image-no-digest", "dockerfile-image-no-tag", "dockerfile-insecure-download", "dockerfile-maintainer-deprecated", "dockerfile-multiple-cmd", "dockerfile-multiple-entrypoint", "dockerfile-multiple-healthcheck", "dockerfile-no-healthcheck", "dockerfile-pip-no-version-pin", "dockerfile-rpm-no-clean", "dockerfile-run-as-root", "dockerfile-run-pipe-shell", "dockerfile-run-sudo", "dockerfile-secret-in-arg", "dockerfile-secret-in-env", "dockerfile-todo-comment", "dockerfile-workdir-relative",
...
func TestCatalogParity(t *testing.T) {
	...
	// Validate key, name, severity, and example-behavior for all 90 misconfiguration rules.
	for _, id := range explicitInventory {
